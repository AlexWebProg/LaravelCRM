...
class MailManagerCreated extends Notification implements ShouldQueue
{
    use Queueable;
    public array $data;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject('Ваш доступ к панели управления ЛК КомпанияМечты')
            ->greeting('Здравствуйте, ' . $this->data['name'] . '!')
            ->line('Для Вас создан доступ к панели управления личным кабинетом в компании "КомпанияМечты":')
            ->line('Email: '.$this->data['email'])
            ->line('Пароль: '.$this->data['password'])
            ->action('Перейти к панели управления', env('APP_URL'))
